...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
                 ...
                 pretrained: str = None):
        assert name in ['swin-t', 'swin-s', 'swin-b', 'swin-l']
        cfgs = configs[name]
...
def build_swin_backbone(args):
    ...
    backbone = Backbone(args.backbone, args.backbone_pretrained, args.backbone_pretrained_path)
    model = Joiner(backbone, position_embedding)
    return model
 

configs = {
    'swin-t': dict(embed_dim=96,
                    depths=[2, 2, 6, 2],
                    num_heads=[3, 6, 12, 24],
                    window_size=7,
                    mlp_ratio=4.,
